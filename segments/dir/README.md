...
|`truncate_with_package_name`|Search for a `package.json` or `composer.json` and prints the `name` field to abbreviate the directory path. The precedence and/or files could be set by `P9K_DIR_PACKAGE_FILES=(package.json composer.json)`. If you have [jq](https://stedolan.github.io/jq/) installed, it will dramatically improve the speed of this strategy. Use `P9K_DIR_SHORTEN_PKG_SEPARATOR` to specify a custom separator between package name and relative subdir. Can optionally fallback to git if `P9K_DIR_SHORTEN_GIT_FALLBACK` is non-zero, taking name from remote `P9K_DIR_SHORTEN_GIT_REMOTE_NAME` or 'origin' if `P9K_DIR_SHORTEN_GIT_USE_REMOTE` is non-zero, otherwise or if remote not available use local folder name |
>>>>>>> 27e2cc5... dir: Add configurable separator between package name and path
